...
module ahb_subordinate_usb (
   ...
   always_comb begin
      ...
      if ((state == ST_IDLE) && hsel && (htrans == HTRANS_NONSEQ))
      ...
      if ((state == ST_IDLE) && active_transfer && !hwrite && !current_error && !start_buf_read) begin
         case (hsize)
            SIZE_BYTE: begin
               ...
               if ((haddr == 4'hC) && bypass_c && (haddr[1:0] == 2'd0))
                  next_hrdata = {24'h0, bypass_data};
               if ((haddr == 4'hD) && bypass_d && (haddr[1:0] == 2'd1))
         ...
         endcase
      end else if ((state == ST_BUF_READ) && (buf_index == buf_count) && (buffer_occupancy != 7'd0)) begin
      ...
      case (state)
         ...
         ST_BUF_WRITE: begin
            ...
            if (buffer_occupancy < 7'd64) begin
               store_tx_data = 1'b1;
               case (buf_lane)
                  2'd0: tx_data = buf_word[7:0];
                  2'd1: tx_data = buf_word[15:8];
                  2'd2: tx_data = buf_word[23:16];
                  default: tx_data = buf_word[31:24];
               endcase
            end
         end
         ST_BUF_READ: begin
   ...
   always_ff @(posedge clk or negedge n_rst) begin
      if (!n_rst) begin
         ...
         if (flush_reg || (get_rx_data && (buffer_occupancy == 7'd1)))
            status_new_data_reg <= 1'b0;
         else if (rx_data_ready && (rx_packet != PID_ACK))
         ...
         case (state)
            ST_IDLE: begin
               ...
               if (prev_valid && prev_write && (prev_size == SIZE_BYTE)) begin
                  ...
                  if ((prev_addr == 4'hD) && (hwdata[7:0] == 8'h01))
                     flush_reg <= 1'b1;
               end

               if (!tx_transfer_active && (tx_packet_reg != TX_NONE))
                  tx_packet_reg <= TX_NONE;
            ...
            ST_BUF_READ: begin
               if (buffer_occupancy != 7'd0) begin
                  case (buf_lane)
                     2'd0: buf_word[7:0] <= rx_data;
                     2'd1: buf_word[15:8] <= rx_data;
                     2'd2: buf_word[23:16] <= rx_data;
                     default: buf_word[31:24] <= rx_data;
                  endcase
